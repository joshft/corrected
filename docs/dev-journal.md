...
## 2026-08-01 — P3 Determinism Attestation (PR2, provenance mechanism + Group G)

PR2 is the middle of a 3-PR arc. PR1 made the determinism check run in an isolated CI lane and emit a structured `RunReceipt`. PR2 builds the frozen mechanism that signs that receipt and later verifies it, plus the live gate-side probe that consumes the verdict, plus the Group G entry-receipt lifecycle. PR3 will commit a real production-identity bundle and flip P3. Nothing in PR2 flips a readiness precondition: no production-identity bundle is committed, so the live P3 probe always resolves the pre-PR3 zero-state (`p3-not-yet-activated`) and readiness stays BLOCKED. The whole point of the PR is that the mechanism is real and tested while the evidence is deliberately absent.

The substrate is a new BCL-only project, `gate/Corrected.Provenance/`, added as the 5th project in `gate/Corrected.Gate.slnx` (INV-022 pins the count). It has three layers: `InToto/` holds the generic in-toto Statement, DSSE envelope, and predicate-type contracts; `Determinism/` holds the `RunReceipt` schema, the `DeterminismAttestation` builder, and a runnable statement emitter; `Entry/` holds the Group G predicate, the `EntryAttestation` builder, a multi-subject statement codec, and an emitter. The signed subject is the determinism `RunReceipt` itself (INV-006). A single canonical serializer builds the exact bytes the signer signs, so the verifier can reconstruct the same bytes and compare them exactly. This byte-equality is load-bearing: cosign's `--check-claims` binds only `subjects[0]` (the receipt digest) and never inspects predicate content, so a mutated predicate that preserves `sha256(receipt)` would slip past cosign. `DeterminismVerifier.Verify` decodes the signed DSSE payload, reconstructs the Statement from the committed receipt, and rejects a mismatch as `statement-reconstruction-mismatch` (INV-010). A second Corrected-side binding check, reached only after cosign accepts the identity, compares the certificate's workflow-SHA to the receipt's `attested_commit` and rejects a mismatch as `cert-workflow-sha-mismatch` (INV-011, the RS-006 "2b" negative). Both are new `rejected` reasons this PR added to `gate/Corrected.Gate/DeterminismVerifyReason.cs`.

The reason enum is the committed source of truth for the fail-closed classifier. Each member carries a `[VerifySeverity]` attribute; the INV-012 totality test reflects over those attributes and asserts the internal→carrier map cell-by-cell, so shrinking or re-pointing the map is a reviewable diff on the enum, not a silent test edit (RS-010, the AP-022/PMB-003 discipline). Only two transient faults (`verifier-unavailable`, `trust-root-or-tool-unreadable`) map to `unavailable`; every other reason and the default branch map to `rejected`. The default-to-`rejected` rule is deliberate — the earlier "any verifier fault → unavailable" default was the seam that armed the RS-001 forged-ENTERED bypass, and it is now closed. `P3Probe.Evaluate` in `gate/Corrected.Gate/Probes.cs` now calls the real verifier with explicit staleness and ancestry inputs (the MA-B wiring), and `StatusRenderer` renders each reason to an actionable line with a `{retryable|hard}` disposition and no `unclassified` fallthrough (INV-021).

Group G (`gate/Corrected.Gate/EntryVerifier.cs` + `EntryIntegrityProbe.cs`) mirrors the determinism verifier but for the phase-entry receipt, whose Statement is multi-subject and self-describing (commit-X plus the P1/P2/P3 preconditions). Its integrity rides on the entry schema's internal subject↔manifest-root binding rather than a separate reconstruction, plus an internal `sha256(receipt)==subjects[0]` re-bind that a fake or compromised cosign cannot satisfy. Its severity map is three-valued (`{Rejected, Unavailable, Absent}`) because entry integrity has an extra `Absent` state, pinned to ordinal 0 as the fail-closed default. `EntryIntegrityProbe` makes the verifier live-callable: an absent entry-activation pointer resolves to `EntryIntegrity.Absent` (the PR2 zero-state, keeping the `src/` ban active); a present pointer drives the real verifier under the production identity. The full Group G activation orchestrator is P2 scope.

Two design points are worth recording. First, the positive verify path is proven only with a throwaway FIXTURE identity, never the production one — a fixture-identity bundle under the production argv is an identity mismatch by construction, so the production ACCEPT branch is genuinely unexercisable until PR3. The fixtures under `test/attestations/fixtures/` are real: they were minted by keyless cosign under the fixture identity and are permanent public Rekor entries, and each verifies offline with a DSSE payload that byte-equals the emitter's statement. Second, scope grew mid-cycle. The PR2 mini-audit (MA-B/C/F plus a "fix everything" hardening track) found that the P3 verify/render layer was still stub-only in production; rather than defer it, the user chose to build the full layer now. That is why `P3Probe` reaches the real verifier in this PR and not a later one — it supersedes the QA round-1 "PR3-deferred" snapshot, and `/cverify` confirmed the build landed and is fail-closed.
