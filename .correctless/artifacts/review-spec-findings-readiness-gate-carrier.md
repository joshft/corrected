...
**Round-6 disposition: applied in v8, EXT6-01..05 → fixed. Post-edit sweep CLEAN (all five EXT6 tags thread through; no residual "documented invocation"/"reproducibility held"/old purity scope/old DD-003 contract phrasing; doc-home verbatim test rebound to `<GATE-SCRIPT>`). Spec is v8; COMMITTED fba1b6c.**

---

## Round 7 — FINAL codex xhigh gate on v8 (2026-07-25)

codex (GPT-5.6, xhigh, read-only full-repo) returned **NO-GO** on v8: 4 BLOCKING + 2 IMPORTANT (EXT7-01..06). Every finding lands in the EXT6 rewrite zones (as predicted), several exactly on the two v8 sub-choices (EXT6-01 command unification → EXT7-01/02; EXT6-05 kernel isolate → EXT7-04/05). Each finding was re-verified against the tree before recording (codex misread one site; noted inline). **Status: pending maintainer disposition. Spec is v8 (fba1b6c); these are candidate v9 repairs.**

### EXT7-01 (BLOCKING → CONFIRMED, narrowed): EXT6-01 command-unification incomplete — an asserted ARCHITECTURE reconcile that did NOT land
**Source**: codex (external) · **Category**: internal-contradiction (claimed-but-unapplied reconcile; EXT4-04 class)
**Location**: `ARCHITECTURE.md:84` (`readiness-build-gate` `test_via` still says: run **the DOCUMENTED command** `dotnet test <AGGREGATOR> --logger "trx;LogFileName=gate.trx"`) vs `readiness-gate-carrier.md:689` (INV-014 CLAIMS "ARCHITECTURE's `test_via` is reconciled to `<GATE-SCRIPT>`"); INV-006:298 Entry names bare `dotnet test` (borderline — integration-harness entry).
**Verification note**: codex also cited INV-017:787 as still-bare — **FALSE**; INV-017:786-789 correctly names `<GATE-SCRIPT>` ("CI executes `<GATE-SCRIPT>` DIRECTLY"). The real, confirmed defect is the stale ARCHITECTURE `test_via` (the spec asserts a reconcile that was never applied) + the ambiguous INV-006 Entry.
**Scenario**: the RED agent + `extract-entrypoints.sh` read `test_via` from ARCHITECTURE as the machine source of truth; they encode bare `dotnet test` as the documented command, shipping the exact v7 split EXT6-01 was meant to close (no guaranteed banner, no guaranteed executed-count guard on the documented command).
**Fix**: make `gate/run-readiness-gate.sh` (`<GATE-SCRIPT>`) the sole documented command in ARCHITECTURE:84 `test_via` and INV-006 Entry; keep the inner `dotnet test … --logger` argv only inside INV-014 (the script's internal step). **Status**: pending

### EXT7-02 (BLOCKING → CONFIRMED): wrapper combined-exit-code + sentinel process-semantics underspecified/untested
**Source**: codex (external) · **Category**: testability / untestable-as-written
**Location**: INV-012:632-635, INV-014:684-698, INV-017:792-795.
**Scenario**: (a) the self-tests (INV-014:696-698) exercise the **TRX guard** given a TRX, not the **wrapper's combined result**. A shell wrapper can return 0 when `dotnet test`=0 + TRX-guard fails + renderer=0; or `set -e` exit on test failure before rendering the FAIL banner (violating INV-012's both-paths visibility). No wrapper-level fixture pins `final_rc == (test_rc==0 && trx_rc==0 && render_rc==0)` or "renderer always runs after `dotnet test`." (b) INV-017:792-795 never says which process **sets** vs **checks** `CORRECTED_GATE_INNER`; as worded it conflates "reference-lane test runs the script under the sentinel" (→ script would early-exit → verbatim test becomes a no-op) with "script sets it for its child `dotnet test`" (the coherent design).
**Fix**: specify + test the state machine — outer script starts sentinel-unset, sets `CORRECTED_GATE_INNER=1` only for its child `dotnet test`, no in-suite test executes the script (doc tests compare bytes only), the out-of-suite lane executes the outer script, final success = `test_rc==0 && trx_rc==0 && render_rc==0`; add wrapper fixtures {nonzero-test+valid-TRX, zero-test+bad-TRX, missing-TRX, happy}. **Status**: pending

### EXT7-03 (BLOCKING → CONFIRMED): EXT6-02's "closed" MSBuild presence policy is not closed, and the scan is unbound from the real compiler inputs
**Source**: codex (external) · **Category**: security / incomplete-enforcement
**Location**: INV-011:526-623 (build phase 530-560; closed policy 551-554; syntax-only scan 561-565; `-getItem:Analyzer` 536-538).
**Scenario**: real uncovered build-time-execution / scan-blind routes remain — (i) committed `Directory.Build.rsp` auto-injects switches (no `-noAutoResponse` required); (ii) a locked build-only `PackageReference` contributes `build`/`buildMultiTargeting`/`buildTransitive` targets via restore-generated `obj/*.nuget.g.props/targets` with no `Reference`/`Analyzer` item and no literal `<Import>`; (iii) a custom project `<Sdk>` triggers implicit imports without a non-SDK `<Import>`; (iv) MSBuild **property functions** in ordinary `PropertyGroup`/`ItemGroup` run at evaluation and can read time/env/culture/files; (v) **`DefineConstants`/`#if` differential** — the real build compiles an active `#if` branch while the symbol-less `CSharpSyntaxTree.ParseText` scan (561) sees it as inactive trivia and never scans it (strongest); (vi) `dotnet build -getItem:Analyzer` returns **evaluation-time** items and runs no target, so target-added analyzers are missed, and the SDK enables built-in analyzers by default with **no defined SDK-default allowlist** → vanilla fixture false-fails.
**Fix**: require `-noAutoResponse`; reject committed response files, unapproved SDK identities, and property functions; inspect locked package assets for build/analyzer content; define the exact permitted SDK-default analyzer set; bind the scan to **post-target** `Compile`/`Analyzer`/references + the build's actual preprocessor symbols (what `Csc` receives), not evaluation-only `-getItem`. Add response-file/buildTransitive/custom-SDK/property-function/conditional-compilation fixtures. **Status**: pending

### EXT7-04 (BLOCKING → CONFIRMED): EXT6-05 kernel isolation is not structurally enforced; the forbidden set does not establish purity
**Source**: codex (external) · **Category**: security / incomplete-enforcement (the EXT6-05 isolate sub-choice)
**Location**: INV-004:225-241 (enforcement 240 = symbol-scan + behavioral check ONLY — **no project-graph assertion**).
**Scenario**: (a) `Corrected.Gate.Kernel` can add a `ProjectReference` to the I/O-capable orchestrator without **using** its symbols and still pass the symbol scan; the "isolated project ⇒ complete scan" claim rests on convention, not enforcement. (b) the forbidden set (232-234) omits direct nondeterminism/I/O counterexamples: `DateTime.Today`, `TimeProvider.System.GetUtcNow()/GetTimestamp()`, `RandomNumberGenerator`, `CultureInfo.CurrentCulture`, `Console`, `System.Net.*`, `Assembly.Location`, `Task.Delay`, `Stopwatch`, mutable static state.
**Fix**: assert the project graph — Kernel has **no** `ProjectReference`/`PackageReference`; Gate/Tests may reference Kernel, never the reverse. Replace the open forbidden list with a closed **allowed**-symbol policy (or materially expand it) + a structural ban on mutable static state; add one fixture per ambient-state/I/O family. **Status**: pending

### EXT7-05 (IMPORTANT → CONFIRMED): the new Kernel project did not ripple through the solution/scope/toolchain contract
**Source**: codex (external) · **Category**: internal-contradiction / incomplete-ripple
**Location**: `Kernel` appears at readiness-gate-carrier.md **only** :54 (EXT6 metadata) + :226 (INV-004); **absent** from Scope :85, Files-touched :107, Packages-Affected :1035, aggregator membership (:85, `ARCHITECTURE.md:84` = "over gate/Corrected.Gate + Tests + Lint"), INV-015/016 restore-lock set, and **0 occurrences in ARCHITECTURE.md**.
**Scenario**: an implementer following Scope/ARCHITECTURE omits Kernel from the solution + lock-membership tests, relying accidentally on a transitive `ProjectReference`; INV-015 states no exact four-project restore/lock set.
**Fix**: add Kernel to the exact `<AGGREGATOR>` membership, Scope/Files/Packages lists, ARCHITECTURE handler + TB-006 paths, and the INV-015/016 restore-and-lock project set; assert exact solution membership (Kernel having no tests of its own is fine). **Status**: pending

### EXT7-06 (IMPORTANT → CONFIRMED terminology; a secondary design sub-point): "single canonical site list" contradicts the B13 exclusion; digest is a validator not a recoverable preimage
**Source**: codex (external) · **Category**: internal-contradiction (wording) + implementation-contract
**Location**: readiness-gate-carrier.md:9 & :926 ("**single canonical site list**") vs DD-003:922-925 (ADR B13 explicitly OUT of the manifest); OQ-A#4 separate-milestone allowance :1026.
**Scenario**: (a) metadata + Packages-Affected call the manifest the single canonical site list for B1–B14 while B13 is outside it and separately enumerated — a meta-test cannot truthfully prove both. (b) `stage_after_sha256` is a **validator**, not a recoverable replacement: if Stage A and Stage B land as separate commits (OQ-A#4), a clean Stage-A checkout holds only a one-way digest + prose, not the Stage-B byte preimage. (Judgment call: EXT6-04 deliberately chose digest-only; (b) may be by-design.)
**Fix**: call it the "canonical **parent-anchor** list," model the ADR B13 edit as a separately named Stage-B obligation (remove it from any manifest-range claim); if separate milestones stay permitted, commit canonical after-span fixtures or a deterministic migration patch while keeping the four-field rows. **Status**: pending

**What held up under round-7 re-review (codex-verified sound in v8)**: EXT6-03 latestPatch (same major/minor/3xx/`>=10.0.302` predicate in INV-011:620, INV-015:721, INV-016:755; `allowPrerelease:false` + 4xx exclusion + no cross-machine SDK-identity claim explicit at :761 + ARCHITECTURE:224); today's ADR has all decision fields but no `status`/`supersedes`/`superseded_by` (ADR-0001:27) matching INV-008's Stage-A `evidence-schema-incomplete` short-circuit, and a P1-flip-with-B13-forgotten is caught by INV-008 false + INV-005 declared-vs-actual (ADR is NOT silently outside all enforcement — resolves the EXT6-04 orphan worry); manifest prose ↔ four-field schema agree locally (:917, :993 — no replacement-string/per-row-stage residue); pinned spike inputs all exist and agree (probe-manifest partition, canonical sample hashes/COMPATIBLE, `ComputeRouteVerdict` duplicate-reject + exact-set); parent OQ-002 really has a discharged contract half + open built-carrier half (parent:1633 — A1 split grounded, not invented); none of the `gate/` projects/script/manifest/`global.json` exist today and v8 consistently treats them as Stage-A deliverables — **absence alone is NOT a PMB-001/002 deadlock** (the from-clean story is sound); EXT6-02's closed-presence policy is scoped to the shipped `src/` closure, so carrier build config does not fail the carrier against itself (:551).

**codex verdict (round 7)**: "v8 is NOT READY FOR TDD; EXT6-01 command/exit/recursion wiring and the EXT6-02/EXT6-05 enforcement closures need one targeted repair pass."

**Round-7 disposition: applied in v9 (2026-07-25; maintainer chose "revise to v9 — fix all six"). EXT7-01..06 → fixed (lightest-correct; the two internal sub-choices resolved toward EXT7-03 = expand-presence-policy + `--noAutoResponse` + parse-with-`DefineConstants` + SDK-default-analyzer allowlist + post-target item binding, and EXT7-04 = expand-forbidden-set + structural project-graph assertion + mutable-static ban).**
- **EXT7-01 → fixed**: ARCHITECTURE:84 `test_via` + INV-006 Entry rebound — the DOCUMENTED command is `<GATE-SCRIPT>` (`gate/run-readiness-gate.sh`), NOT bare `dotnet test`; INV-006's orchestration assertion clarified to run INSIDE the `dotnet test` suite, not as a shell-out. (Also folded the Kernel project into ARCHITECTURE:84's aggregator enumeration.)
- **EXT7-02 → fixed**: INV-014 pins the wrapper exit-code state machine (`final_rc == test_rc==0 && trx_rc==0 && render_rc==0`, no `set -e` short-circuit, renderer ALWAYS runs after `dotnet test`) + four script-level wrapper fixtures {nonzero-test+valid-TRX, zero-test+bad-TRX, missing-TRX, happy}; INV-017 gives the `CORRECTED_GATE_INNER` sentinel a defined set/check owner (outer script exports it only for its child `dotnet test`; reference lane runs the outer script sentinel-UNSET; no in-suite test executes the script; doc tests compare bytes only).
- **EXT7-03 → fixed**: INV-011 build phase invoked with `--noAutoResponse`; presence policy broadened to reject committed `Directory.Build.rsp`/`MSBuild.rsp`, non-default `<Sdk>`/`Sdk=` identities, MSBuild property functions, and `build`/`buildTransitive` package assets in the locked `packages.lock.json`; `Analyzer`/`Compile` extraction bound to a post-target `-getItem` query with an EXACT committed SDK-default `Analyzer` allowlist (no vanilla false-fail); the syntax scan bound to the build's actual `DefineConstants` so a live `#if` branch is not invisible; response-file/custom-SDK/property-function/buildTransitive/`#if`-differential/SDK-default-PASS fixtures added.
- **EXT7-04 → fixed**: INV-004 given a structural project-graph meta-test (Kernel declares NO `ProjectReference`/`PackageReference`, one-way Gate/Tests→Kernel); forbidden set materially expanded (`DateTime.Today`, `TimeProvider`, `Stopwatch`, `RandomNumberGenerator`, `CultureInfo.Current*`, `Console`, `System.Net.*`, `Assembly.Location`, `Thread.Sleep`/`Task.Delay`, `GC`, `Environment.TickCount*`) + a mutable-static-field ban + per-family fixtures; the "references nothing I/O-capable" overclaim corrected to "I/O-free is a property of the Kernel's own code" (BCL still exposes I/O — hence a symbol scan, not a reference scan).
- **EXT7-05 → fixed**: `Corrected.Gate.Kernel` rippled through Scope, Files-touched, Complexity abstractions, Packages-Affected, the INV-015 exact four-project restore/lock set (with a solution-membership meta-test), and ARCHITECTURE (handler + TB-006 exercised-at + aggregator).
- **EXT7-06 → fixed**: "single canonical site list" → "canonical **parent-anchor** site list" at all three sites; B13 modelled as a separately-named Stage-B obligation (NOT a manifest row); the digest-is-validator-not-preimage sub-point closed honestly (the manifest is a validator/guard; Stage-B bytes come from the spec's normative per-site prose, the `stage_after_sha256` certifies them, sufficient even for separate Stage-A/B commits).

Post-edit stale-language sweep CLEAN: `single canonical site list` → 0; every "documented command" reference resolves to `<GATE-SCRIPT>` (no bare-`dotnet test`-as-documented-command residue); ARCHITECTURE:84 aggregator now enumerates all four projects; version → v9, "seven rounds". **Spec is v9; UNCOMMITTED (awaiting maintainer commit/advance decision).**

---

## Round 8 — codex xhigh VERIFICATION gate on v9 (2026-07-25)

codex (GPT-5.6, xhigh, read-only) run as a targeted verification pass — did the EXT7 rewrites introduce a new contradiction / unsatisfiable-from-clean requirement? Returned **NO-GO**: 4 BLOCKING + 2 IMPORTANT + 1 MINOR (EXT8-01..07), ALL re-verified against the tree. The round did its job: it caught **one genuinely-new contradiction the Kernel split exposed (EXT8-03)**, **one regression introduced by the round-7 EXT7-06 edit (EXT8-06)**, one real cross-invariant interaction (EXT8-05), one insufficient round-7 fix (EXT8-04), two fixture/denylist completeness gaps (EXT8-01/02), and one minor stale shorthand (EXT8-07). **Status: pending maintainer disposition. Spec is v9 (UNCOMMITTED); these are candidate v10 repairs.**

**What held up (codex-verified sound in v9)**: EXT7-01 command unification (ARCHITECTURE names `<GATE-SCRIPT>` as the documented command, `dotnet test` only its internal step; INV-006 runs the orchestration assertion inside the suite without shelling out); the sentinel ownership (script sets it only for the child suite, no in-suite test executes the script, byte-comparison doc tests don't recurse); INV-012/AP-020 bind stdout+verbatim execution to `<GATE-SCRIPT>`; EXT7-03's residual boundary (risky build logic may execute before rejection, presence policy explicitly scoped to the shipped `src/` closure; `--noAutoResponse` + response-file rejection is valid belt-and-suspenders; post-target `-getItem` is implementable and stays syntax-only, not in-process MSBuild); a zero-`PackageReference` Kernel does NOT conflict with per-project locked restore; all exact aggregator enumerations agree on the four projects and Kernel need not own tests (TRX floor is from the discovered suite); B13 is correctly outside the parent-anchor manifest and still enforced by INV-008 (today's ADR has one `adr_lint` block lacking status/supersession → matches Stage-A `evidence-schema-incomplete`).

### EXT8-01 (BLOCKING → CONFIRMED): EXT7-02's wrapper fixtures do not enforce the `render_rc` term
**Source**: codex (external) · **Category**: test-coverage / untestable-as-written
**Location**: INV-014 — readiness-gate-carrier.md:754-777.
**Scenario**: the formula requires success only when `test_rc && trx_rc && render_rc` are all zero (:754-760), but the four wrapper fixtures {nonzero-test+valid-TRX, zero-test+bad-TRX, missing-TRX, happy} never force the renderer itself nonzero (:773-777). A wrapper exiting on `test_rc || trx_rc` passes all four yet exits zero when tests+TRX pass but rendering fails — the suite doesn't enforce its own state machine; "FAIL text on any failure" is undefined when the renderer is the failing component.
**Fix**: add a `zero-test + valid-TRX + renderer-nonzero → wrapper nonzero` fixture, with a shell-owned fallback failure message if the renderer cannot emit one. **Status**: pending

### EXT8-02 (BLOCKING → CONFIRMED): EXT7-04's expanded denylist still does not establish Kernel purity
**Source**: codex (external) · **Category**: security / incomplete-enforcement
**Location**: INV-004 — readiness-gate-carrier.md:247-275, enforcement :276-283.
**Scenario**: the scan bans selected BCL members but misses `Assembly.LoadFrom`/`AssemblyLoadContext.LoadFromAssemblyPath` (:259-266) — Kernel can load a file-backed assembly while passing the project-graph + symbol rules; the no-file-touch behavioral test can miss an input-gated branch. The mutable-state rule permits `static readonly` fields (:267-268), but a `static readonly List<T>`/`Dictionary<K,V>` is still mutable and can change later verdicts while passing the field-modifier test — contradicting the "denylist covers the reachable I/O/nondeterminism families" claim (:270-273).
**Fix**: use a closed allowed-symbol/member policy for the tiny Kernel, OR independently ban every static field except `const`/a narrowly-enumerated deeply-immutable type AND add reflection/assembly-loading to the forbidden set; add negative fixtures for reflection-load and `static readonly` mutable collections. **Status**: pending

### EXT8-03 (BLOCKING → CONFIRMED, new contradiction from the split): `ReadinessBlock` unconstructible under INV-003's access contract
**Source**: codex (external) · **Category**: internal-contradiction (cross-assembly access)
**Location**: INV-003 + INV-004 — readiness-gate-carrier.md:227-234, :242-248; placement :132-136.
**Scenario**: `ReadinessBlock` is a kernel input, so (EXT6-05, one-way parser→Kernel) it must be DEFINED in `Corrected.Gate.Kernel`. But INV-003:229-232 requires it be built only through a **private** validation-gated factory with **no public constructor** (a reflection test asserts `GetConstructors(Public)` is empty). A parser in the separate `Corrected.Gate` assembly cannot call a private Kernel member; defining the type in Gate instead needs the prohibited Kernel→Gate reverse reference. The EXT6-05/EXT7-04 project split created the contradiction.
**Fix**: pin the cross-assembly construction API — a private constructor + a public validation-performing `TryCreate` (still no public *constructor*, still no settable property), or an `internal` factory + a named `InternalsVisibleTo("Corrected.Gate")` contract + structural test. Update INV-003's "private factory / empty public ctor" wording to the chosen accessible-but-validation-gated form. **Status**: pending

### EXT8-04 (BLOCKING → CONFIRMED, round-7 fix insufficient): EXT7-06 still has no recoverable Stage-B byte preimage for separate commits
**Source**: codex (external) · **Category**: satisfiable-from-clean / migration-contract
**Location**: DD-003 — readiness-gate-carrier.md:1005-1014, Stage-B descriptions :1049-1070, separate-commit option :1119-1122.
**Scenario**: Stage A may land separately (OQ-A#4) carrying only `stage_after_sha256`. B1 uses the `<registered id>` placeholder (:1049-1050), B3 is a semantic prose rewrite (:1051-1053), B9/B11 are broad reconciliations (:1060-1065) — corresponding to real parent text (phase-0-1-worker.md:138-142, :212-228, :1066-1096, :1594-1598). A maintainer cannot derive a SHA-256 preimage from those; computing the digest at Stage B from newly-chosen prose makes the "validator" certify the same bytes that authored its expected digest (circular). B13 has the same problem (today's ADR has none of the three migration fields; ADR-0001:27-40). My round-7 "validator not source; bytes from normative prose" framing did not make the bytes recoverable.
**Fix**: commit canonical after-span fixtures or a deterministic migration patch in Stage A, derive `stage_after_sha256` from those committed bytes, and provide a separate canonical ADR patch/fixture for B13 — OR require Stage A + Stage B to land atomically (one commit) so a separate-Stage-A-only checkout never exists. **Status**: pending

### EXT8-05 (IMPORTANT → CONFIRMED): EXT7-03's exact analyzer allowlist conflicts with the permitted SDK range
**Source**: codex (external) · **Category**: internal-contradiction (EXT6-03 latestPatch × EXT7-03 analyzer pin)
**Location**: INV-011/015/016 — readiness-gate-carrier.md:587-591, :803-817, :839-857; ARCHITECTURE.md:224-235.
**Scenario**: INV-011 commits ONE exact SDK-default `Analyzer` set called "SDK-bump-coupled," but `latestPatch` permits any installed 10.0.3xx ≥ 10.0.302 (:845-857) WITHOUT changing `global.json`'s requested version, so the DD-006 bump procedure is not triggered. A newer permitted patch that adds/removes a default analyzer makes a vanilla skeleton false-fail despite being in-band. Widened by permitting three SDK identities (base/Web/Razor) while naming one analyzer set (:605-608).
**Fix**: compute the SDK-default `Analyzer` set DYNAMICALLY at gate time from a baseline empty project built under the SAME resolved SDK (closure Analyzer set minus baseline = the non-default set that must be empty/allowlisted) — sidesteps the floating-patch problem; OR key committed analyzer manifests by exact resolved SDK version+identity and fail "unregistered SDK patch" when absent. Do not use one union allowlist across SDK identities. **Status**: pending

### EXT8-06 (IMPORTANT → CONFIRMED, regression from EXT7-06): DD-003's "no local site list" meta-test contradicts the Packages-Affected text
**Source**: codex (external) · **Category**: internal-contradiction (self-inflicted in round 7)
**Location**: DD-003 + Packages Affected — readiness-gate-carrier.md:1019-1025, :1139-1145.
**Scenario**: DD-003 requires the Packages-Affected bullet to hold ONLY the manifest reference and "NO local site list" (:1022-1025), but the round-7 EXT7-06 edit made that bullet enumerate `A1–A6`, `B1–B12 + B14`, B4, and B13 (:1139-1145) then claim it does not re-enumerate. A real no-local-list test fails this text. The `B1–B12` shorthand also includes B4 before saying B4 moved to A6.
**Fix**: remove all A/B ranges from Packages-Affected; keep the parent-anchor enumeration solely in DD-003/the manifest and describe the ADR obligation in its separate ADR bullet without parent-anchor IDs. **Status**: pending

### EXT8-07 (MINOR → CONFIRMED): stale shorthand outside the exact four-project enumerations
**Source**: codex (external) · **Category**: internal-contradiction (stale shorthand)
**Location**: Context + TB-004 — readiness-gate-carrier.md:84-85 (Context shorthand), :1133 (`lockfile` singular); ARCHITECTURE.md:268 (TB-004 names only `gate/Corrected.Gate/packages.lock.json` while INV-015 requires a lock per project).
**Fix**: update the Context shorthand; enumerate or reference the four per-project locks in TB-004; make `lockfile` plural at :1133. **Status**: pending

**codex verdict (round 8)**: "v9 is NOT READY FOR TDD; EXT7-02 wrapper coverage, EXT7-04 Kernel purity/construction, and EXT7-06's clean-checkout migration preimage need one more targeted fix."

**Round-8 disposition: applied in v10 (2026-07-25; maintainer chose "revise v10 fixing all 7, then round 9 gate"). EXT8-01..07 → fixed (sub-choices resolved: EXT8-03 → public `TryCreate`; EXT8-04 → committed after-span fixtures + atomic Stage-B flip; EXT8-05 → dynamic baseline-diff).**
- **EXT8-01 → fixed**: INV-014 gains a FIFTH wrapper fixture (renderer-nonzero → script exits non-zero + shell-owned fallback FAIL line) forcing the `render_rc` term the prior four left unenforced.
- **EXT8-02 → fixed**: INV-004 adds `Assembly.{Load,LoadFrom,LoadFile}`/`AssemblyLoadContext` to the forbidden set; tightens the static-field ban to reject `static readonly` MUTABLE collections (only `const`/deeply-immutable-readonly permitted); reframes the denylist as defense-in-depth behind the real backstop — a behavioral determinism check run under a mutated culture/clock/cwd + the project-graph bound; fixtures for reflection-load + `static readonly` collection.
- **EXT8-03 → fixed**: INV-003's factory is now a `public static` validation-performing `TryCreate` (private constructor retained, no settable property, reflection test keeps `GetConstructors(Public)` empty) so the `Corrected.Gate` parser can construct the Kernel-resident `ReadinessBlock` across the assembly boundary; `internal` + `[InternalsVisibleTo]` noted as the equivalent alternative.
- **EXT8-04 → fixed**: DD-003 commits a canonical after-span fixture per deterministic anchor (under `gate/Corrected.Gate.Tests/manifests/after-spans/`, site A5) with `stage_after_sha256` derived FROM it (recoverable preimage, non-circular); the Stage-B flip MUST land atomically (flip + all edits + B13 in one commit); B1's evidence id is validated structurally (`structural-match`, not digest-pinned) since it is assigned at GREEN.
- **EXT8-05 → fixed**: INV-011's SDK-default `Analyzer` set is computed DYNAMICALLY from a baseline empty project built under the SAME resolved SDK (closure Analyzer set minus baseline = non-default set to reject), sidestepping `latestPatch` band drift and covering all permitted SDK identities uniformly — no committed const, no vanilla false-fail.
- **EXT8-06 → fixed**: the round-7 A/B enumeration removed from the Packages-Affected parent bullet (it now holds ONLY the manifest reference); the per-site enumeration lives solely in DD-003, honoring DD-003's own "no local site list" meta-test.
- **EXT8-07 → fixed**: the Context shorthand now names all four projects; ARCHITECTURE TB-004 and the Packages-Affected lock reference are a per-project `packages.lock.json` (one per gate project).

Post-edit stale-language sweep CLEAN: version → v10, "eight rounds"; no lingering committed-const analyzer allowlist / "private validation-gated factory" / Packages-Affected A/B enumeration / "one-way digest sufficient" / old mutable-static wording / lockfile-singular; DD-003 retains its B-site enumeration. **Spec is v10; UNCOMMITTED. Maintainer requested a round-9 verification gate before commit/advance.**

---

## Round 9 — codex xhigh VERIFICATION gate on v10 (2026-07-25)

codex (GPT-5.6, xhigh) run as a targeted verification pass on the EXT8 fixes. Returned **NO-GO**: 3 BLOCKING + 4 IMPORTANT + 1 MINOR (EXT9-01..08), all re-verified. **DECISIVE non-convergence signal: SIX of the eight (EXT9-01/02/04/05/06/08) are direct consequences of the round-8 EXT8 fixes** — each fix that closed one implementation-contract hole opened another. Findings EXT9-03 (purity can't be proven in prose vs `[DllImport]`) and EXT9-06 (atomic-commit can't be proven by a tree-state gate) name the root cause: the spec is litigating IMPLEMENTATION/CI-mechanism decisions in prose. **Status: pending maintainer disposition; recommendation = fix the honest/cheap items, fold the rest into the RED charter, ADVANCE to TDD (stop the gate loop).**

**What held up (codex-verified sound in v10)**: EXT8-01 five-fixture render_rc coverage + shell-owned fallback implementable; committing after-span fixtures at Stage A does NOT affect parent anchor discovery / finite scan (scoped to phase-0-1-worker.md), and B3/B9/B11 fall under the universal deterministic-anchor fixture rule; the one-commit Stage-B requirement does not contradict the one-PR/two-PR option; public validated construction does not bypass the invariant (factory validates, ctors private, props non-settable, no stale private-factory site); pre-migration state coherent (P1 false/null, ADR lacks optional fields); EXT8-07 four-project enumeration symmetric across carrier/solution/locks/TB-004/Packages-Affected.

### EXT9-01 (BLOCKING → CONFIRMED, from EXT8-04): DD-003 cannot represent B1 under its own closed manifest schema
**Location**: readiness-gate-carrier.md:1041, :1053, :1131. **Scenario**: B1's evidence needs a `structural-match` marker (EXT8-04) but the closed schema is exactly `{id,file,stage_before_sha256,stage_after_sha256}` — adding the marker violates the schema; omitting it leaves B1 unrepresentable. **Fix**: define a closed DISCRIMINATED UNION — digest rows (naming a canonical after-span fixture) vs a structural row (B1: names its exact predicate, omits the after-digest). **Status**: pending

### EXT9-02 (BLOCKING → CONFIRMED, from EXT8-06 half-fix): Metadata `Impacts` still self-fails the "no local site list" meta-test
**Location**: readiness-gate-carrier.md:6, :9, :1063, :1182. **Scenario**: DD-003's meta-test requires BOTH `Impacts` and the Packages-Affected bullet hold only the manifest reference; Packages-Affected now complies but `Impacts` still names `B13`, so a no-`A\d+`/`B\d+` test fails v10 from clean. **Fix**: remove the B13 carve-out from `Impacts`; keep it only in DD-003 + the dedicated ADR bullet. **Status**: pending

### EXT9-03 (BLOCKING → CONFIRMED overstated-claim): INV-004's "real backstops" still do not PROVE no-I/O
**Location**: readiness-gate-carrier.md:267, :281, :295, :297, :307. **Scenario**: a BCL-only `[DllImport]` calling libc reads a stable non-fixture file, ignoring the result — no forbidden `System.IO`/package/project ref; determinism holds under culture/clock/cwd; the fixture-file assertion is evaded. Both named backstops pass while the kernel does I/O. (Kernel is first-party, so this is over-strict as a *threat*, but the *completeness claim* is false.) **Fix** (proportionate): DOWNGRADE the claim — the check catches ACCIDENTAL I/O in first-party kernel code; a malicious P/Invoke is out of scope for a purity lint (not a sandbox boundary). Add `[DllImport]`/`extern` to the forbidden declaration set (INV-011 already rejects `extern`) as cheap defense-in-depth. Do NOT claim proof of no-I/O. **Status**: pending

### EXT9-04 (IMPORTANT → CONFIRMED, from EXT8-02): "deeply immutable" is not recursively defined
**Location**: readiness-gate-carrier.md:290, :291, :307. **Scenario**: `static readonly ImmutableArray<List<int>>` (immutable outer, mutable inner) / a record containing `List<T>` slip the direct-collection fixture. **Fix**: recursive predicate — every generic arg + every record field/property must itself satisfy the closed immutable predicate; add nested-wrapper + record-containing-collection fixtures. **Status**: pending

### EXT9-05 (IMPORTANT → CONFIRMED, from EXT8-05): dynamic Analyzer baseline not bound to isolated restore / one resolved SDK
**Location**: readiness-gate-carrier.md:616, :621, :625, :685, :845; ARCHITECTURE.md:268. **Scenario**: a runtime baseline outside `gate/` doesn't inherit `gate/NuGet.Config`, has no committed lock, and two invocations under `latestPatch` can resolve different patches. **Fix**: pin a committed baseline fixture under `gate/` with lock/config discipline; assert baseline+closure report the same captured `NETCoreSdkVersion`; skip baseline after the zero-project vacuous short-circuit. **Status**: pending

### EXT9-06 (IMPORTANT → CONFIRMED, from EXT8-04): "one Stage-B commit" is not enforceable by a tree-state gate
**Location**: readiness-gate-carrier.md:1050, :1141, :1164. **Scenario**: a PR with a red partial-flip commit then a green complete commit is indistinguishable from atomic at HEAD; the digest gate rejects a partial TREE but cannot prove per-commit atomicity. **Fix**: redefine the guarantee as **atomic accepted TREE STATE** (which the mixed-set fail-closed guard already delivers) and DROP the unenforceable per-commit claim — OR add history-aware CI over the Stage-B commit range. **Status**: pending

### EXT9-07 (IMPORTANT → CONFIRMED, pre-existing): the constructor reflection test is vacuous as written
**Location**: readiness-gate-carrier.md:251. **Scenario**: `GetConstructors(BindingFlags.Public)` omits `BindingFlags.Instance` → can return empty even with a public instance ctor, so the bypass check greens with an unchecked public ctor. **Fix**: `GetConstructors(BindingFlags.Instance | BindingFlags.Public)` (or the parameterless-public-ctor overload), applied to every protected domain type. **Status**: pending

### EXT9-08 (MINOR → CONFIRMED, from EXT8-03): type ownership + accessibility underdetermined
**Location**: readiness-gate-carrier.md:147, :244, :247, :249, :253; ARCHITECTURE.md:83. **Scenario**: the factory rule names `ReadinessBlock`+`AdrLintBlock`; the cross-assembly rationale names `ReadinessBlock`+`ProbeResult`; `AdrLintBlock`'s assembly is unpinned; "MUST be public" then "internal+InternalsVisibleTo equivalent" — underdetermined. **Fix**: pin the assembly + construction rule for ALL THREE types and select ONE accessibility form. **Status**: pending

**codex verdict (round 9)**: "v10 is NOT READY FOR TDD; DD-003's manifest/atomicity contract, INV-004 purity enforcement, EXT8-06 Metadata cleanup, and the Analyzer-baseline binding need one targeted fix round."

**MAINTAINER-AGENT ASSESSMENT (non-convergence)**: rounds 7→8→9 each resolved N findings and surfaced ~N more, with round 9 tracing 6/8 findings directly to the round-8 fixes. The residual is implementation-contract detail that RED tests + real CI will validate far better than further prose gates. Recommendation: apply the honest/cheap subset (EXT9-02/07/08 + the EXT9-01 discriminated union + reframe EXT9-03/06 honestly), fold EXT9-04/05 into the RED test charter, then ADVANCE review-spec → tdd-tests.

**Round-9 disposition: applied in v11 (2026-07-25; maintainer chose "fix all 8 in v11, then HOLD"). EXT9-01..08 → fixed. No advance, no further gates — held uncommitted at maintainer request.**
- **EXT9-01 → fixed**: DD-003 manifest schema is now a closed discriminated union on `kind` — digest rows `{id,file,kind:"digest",stage_before_sha256,stage_after_sha256}` vs a structural row `{id,file,kind:"structural",stage_predicate}` for B1's GREEN-assigned evidence id.
- **EXT9-02 → fixed**: the B13 carve-out removed from Metadata `Impacts` (symmetric with the Packages-Affected "no local site list" meta-test); the ADR obligation is described only in DD-003 + the ADR bullet.
- **EXT9-03 → fixed**: INV-004's purity claim honestly SCOPED to catching accidental first-party I/O (a `[DllImport]` P/Invoke bypass is out of scope — the kernel is not a sandbox boundary); `extern`/`[DllImport]` added to the forbidden kernel-declaration set as cheap defense-in-depth.
- **EXT9-04 → fixed**: "deeply immutable" is now a RECURSIVE predicate (every generic arg + record field/property must itself satisfy it); nested-wrapper (`ImmutableArray<List<int>>`) + record-containing-collection fixtures added.
- **EXT9-05 → fixed**: the dynamic Analyzer baseline is a committed `gate/Corrected.Gate.Tests/fixtures/analyzer-baseline/` project under `<clear/>` + locked restore; the gate asserts baseline+closure report the same `NETCoreSdkVersion`; baseline construction skipped after the zero-project vacuous short-circuit.
- **EXT9-06 → fixed**: the migration guarantee redefined as **atomic accepted TREE STATE** (the P1-derived selector + mixed-set fail-closed guard already deliver it); the unenforceable per-commit "one commit" claim dropped (a tree-state gate cannot see history); B-site header + Staging invariant reworded to tree-state.
- **EXT9-07 → fixed**: the reflection test is now `GetConstructors(BindingFlags.Instance | BindingFlags.Public)` (the bare `Public` form omitted instance ctors and vacuously passed).
- **EXT9-08 → fixed**: INV-003 pins all three protected types' homes (`ReadinessBlock`/`ProbeResult` in Kernel, `AdrLintBlock` in Gate) and selects the SINGLE `public static TryCreate` form for all three (no `internal`+`InternalsVisibleTo` variant).

Post-edit stale-language sweep CLEAN: version → v11, "nine rounds"; no lingering `GetConstructors(Public)`-alone / bare `structural-match` / unqualified "MUST land atomically" / "B13" in Impacts / stale 4-field-only schema claim; discriminated union present; DD-003 "each digest row stores…" qualified for the structural exception. **Spec is v11; HELD UNCOMMITTED (maintainer: no advance, no further gates).**

**NOTE (convergence, for the record)**: this closes a 9-round arc. The maintainer-agent assessment stands — the residual after v11 is implementation-contract detail best validated by RED tests + real CI, not further prose gates. When work resumes, the recommended next step is to commit v11 and advance `review-spec → tdd-tests`, treating any remaining depth (e.g. EXT9-04/05 mechanics) as concrete RED obligations rather than re-gating the spec.
