...
- **Enforcement**: test assertion (local `dotnet test` until CI — EA-004/DD-005) — provisioning three-state + double-run tests; a test asserting `.gitignore` covers the `out/` area; README existence **plus verbatim entry-point execution**: a test runs each documented operator command *exactly as the README tells the operator to type it* — same working directory and same relative/absolute `argv[0]` form, under the hardened `env -i … bash -p` invocation — and asserts the controller reaches and launches its inner controller (no exit-127 / "No such file"). A README keyword-presence check does **not** satisfy this, and neither does a behavioral test that normalizes the invocation to an absolute script path, a fixed cwd, or an injected environment — a normalized proxy cannot catch form-specific launch defects (a relative `BASH_SOURCE`/`$0`/argv path reused after a `cd`; cwd assumptions; env stripping) — AP-020/AP-011. The authoritative full-regression net is the DF-001 clean-checkout (`rm -rf out`) CI job. Summary-line presence asserted in the child-process integration tests.
...
- **DD-005 — CI permanence with a structural carrier** (resolved OQ-001; hardened per RS-011a): when CI is introduced, the spike suite becomes a permanent CI job. Structural carriers, not prose: (1) a **stub CI workflow file committed under `spikes/dafny-compat/ci/`** that the CI feature must wire (its existence is the handoff artifact); (2) the Correctless workflow test command configured to run the suite now (EA-004); (3) this obligation registered NOW as **DF-001** in `.correctless/meta/deferred-findings.json` (codex F15/R2-12: registered at spec time, not promised for later), which the CI feature must discharge. **The wired job MUST run FROM-CLEAN** (PMB-002): `rm -rf out` / a fresh checkout with no accumulated `out/runid-*` run roots or `out/current` pointer — caching run roots between runs is forbidden (only `out/cache` provisioning artifacts may be cached, restored into an otherwise-empty `out/`). The from-clean run is the **structural detector for the AP-021 circular-gate class: no suite/gate test may require a prior successful run of the same suite, and every gate must be provable green from a single clean checkout** — a warm `out/` lets a suite pass on leaked prior-run state (AP-010), and this net's absence let *both* post-merge bugs (PMB-001 exit-127 entry point, PMB-002 the MaIdR22 circular suite-receipt gate) escape local `dotnet test`. The committed stub encodes the from-clean requirement (QA-023 note 5), test-asserted (`StubCiWorkflow_Committed_AsHandoffArtifact`). Until CI lands, enforcement is local `dotnet test` (EA-004).
