...
### AP-021: Suite/gate test whose precondition is only satisfiable by a prior successful run of the same suite (non-bootstrappable circular gate) `[local: PMB-002]`
- **What went wrong (local)**: the mini-audit added `MaIdR22_SuiteReceipt_ProduceSideBinding_AndPathMatchesConstant` (`MiniAudit2OperatorTests.cs`) as the PRODUCE-side class fix for finding MA-ID-R2-2 (suite-receipt binding). Instead of validating the **current** run's receipt (the controller/aggregator product of *this* run), the test enumerates prior run roots on disk (`Directory.EnumerateDirectories(out, "runid-*" | "regen-*")`) and asserts that **at least one prior run's** `receipts/suite-receipt.json` carries `suite_exit==0`. But that green receipt is only produced by a **successful suite**, and this test is a **member of that suite** — so from a clean checkout (`rm -rf out`) no prior green receipt exists, the test fails, the suite exits non-zero, the receipt this run writes records `suite_exit != 0`, and the next run still finds no green prior receipt. The "already green" fixpoint is unreachable from clean; the gate **deadlocks**. On a developer tree that already carried an accumulated green `out/`, the test passed for the wrong reason (leaked prior-run state — AP-010), so the mini-audit round, `cverify`, and the green suite all saw pass. It regressed the spec's own **RS-010** principle — the aggregator "derives its expected report set from the manifest's committed route plan (**never from reports found on disk**)" and is "**violated when** the aggregator iterates reports found on disk" — by committing that forbidden disk-iteration/prior-run dependency **inside a suite test**. Same class as the QA-001+QA-024 evidence-sample deadlock (a committed-sample gate that converges only from an already-green / ancestor-bound state and cannot reconverge from a rewritten or clean state).
- **How to catch it**: every suite/gate test must be provable **green from a single run of a clean checkout** (`rm -rf out` / fresh clone) with no accumulated state — a from-clean run is the structural detector for this whole class, and it must be an **executable gate, not a deferred CI promise** (DF-001, the `rm -rf out` clean-checkout job, was deferred/unwired, and its absence let *both* this bug and PMB-001 escape — two post-merge bugs sharing one missing net). A test that reads its subject from a shared, mutable, prior-run location (live `out/`, `out/current`, on-disk receipts/reports it did not itself produce this run) is checking **self-produced state**: it passes only via leaked prior-run state and cannot bootstrap. Any invariant about a run's **own** product must bind to the **current** run's artifact — produced within this run by the controller/aggregator and handed to the test through the run context (`RunContext`) — never discovered by enumerating prior run roots on disk. A gate whose only satisfying precondition is a prior successful execution of the same gate is a circular/bootstrap deadlock and a blocking finding. Related: AP-010 (test passes on leaked prior state — the wrong-reason sibling), AP-016 (fresh/clean-state testing of stateful code), AP-018 (self-produced completeness claim), AP-005 (a frozen/committed gate with no legitimate path to reconverge).
- **Frequency**: 2 findings across 1 feature (dafny-compat-spike: MaIdR22 suite-receipt gate; QA-001+QA-024 evidence-sample git-binding deadlock). Below the 3-feature promotion threshold — no ARCHITECTURE.md promotion.
